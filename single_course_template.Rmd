...
ottrpal::borrow_chapter(doc_path = "chunks/aboutCourse.md",
                        tag_replacement = list("{COURSE_DESCRIPTION}" = grab_data(course_info, params$row_number, "description", pastequotes = FALSE), "{COURSE_CATEGORY}" = grab_data(course_info, params$row_number, "Category", pastequotes = FALSE), "{COURSE_LAUNCH}" = grab_data(course_info, params$row_number, "launch_date", pastequotes = FALSE)))
```

```{r course_formats}
#Call the formatsFullCourse.Rmd
ottrpal::borrow_chapter(doc_path = "chunks/formatsFullCourse.Rmd",
                        tag_replacement = list("{BOOKDOWN_LINK}" = grab_data(course_info, params$row_number, "BookdownLink", pastequotes = FALSE), "{GITHUB_LINK}" =  grab_data(course_info, params$row_number, "GithubLink", pastequotes = FALSE), "{COURSERA_LINK}" = grab_data(course_info, params$row_number, "CourseraLink", pastequotes = TRUE), "{LEANPUB_LINK}" =  grab_data(course_info, params$row_number, "LeanpubLink", pastequotes = TRUE)))
```

```{r course_aud}
#Call the audienceCourse.Rmd
ottrpal::borrow_chapter(doc_path = "chunks/audienceCourse.Rmd",
                        tag_replacement = list("{FOR_SLIDE_LINK}" = grab_data(course_info, params$row_number, "for_slide"), "{COURSE_AUDIENCE}" = grab_data(course_info, params$row_number, "BroadAudience", pastequotes = TRUE)))
```
